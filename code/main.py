...
def PredictScore(train_drug_dis_matrix, drug_matrix, dis_matrix, seed, epochs, emb_dim, dp, lr, adjdp):
    ...
    adj = constructHNet(train_drug_dis_matrix, drug_matrix, dis_matrix)  # adj.shape=(867,867)
    ...
    model = HDGATModel(placeholders, num_features, emb_dim,
                     features_nonzero, adj_nonzero, train_drug_dis_matrix.shape[0], name='HDGAT')
...
if __name__ == "__main__":
    ...
    # drug_dis_matrix=np.loadtxt('../data/therapeutic.csv',delimiter=',')
    epoch = 400
    emb_dim = 64
    lr = 0.01
    adjdp = 0.6
